...
@props([
    ...
    'required' => false,
    'disabled' => false,
    'placeholder' => __('Choose an image or drag & drop it here. <span class="filepond--label-action"> Browse file </span>'),
])
                  ...
                  await @this.upload('{{ $wireModelAttribute }}', file, async (response) => {
                    let validationResult  = await @this.call('validateUploadedFile', response);
                        ...
                        if (validationResult === true) {
                            ...
                            await @this.revert('{{ $wireModelAttribute }}', response, load);
